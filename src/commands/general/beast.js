...
export default class BeastCommand extends Command {
  ...
  async* args(): Object {
    const x = yield {
      type: Argument.range('string', 1, 15),
      match: 'rest',
      default: null,
      prompt: {
  ...
  async getWiki(name: string): Array<any> {
    const body = await go.get(`https://runescape.wiki/api.php?action=query&format=json&prop=extracts%7Cpageimages%7Cinfo&iwurl=1&generator=search&formatversion=2&exsentences=5&exintro=1&explaintext=1&piprop=original&inprop=url&gsrsearch=${encodeURI(name)}&gsrlimit=1`,
